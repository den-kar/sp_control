...
# -------------------------------------
def get_min_hours(df_ee, ref_names, contract, name):
  if 'TE' in contract or name not in ref_names:
...
def load_decrpyted_xlsx(file_path):
  ...
  while tries > 0:
    try:
      ...
      df = read_excel(decrypted).rename(columns=lambda x: str(x).strip())
      break
    except:
      if tries:
        tries -= 1
    ...
    for y in range(height - 1, -1, -1):
      ...
      if pixel_color not in COLOR[LINE]:
        ...
        if y <= rows[-1] - 5:
          ...
          if DEV == 4:
            print(f'narrow bottom row, replace row[0]= {height} -> {y}')
          rows[0] = y
      else:
        vert_line_cnt += 1
        if vert_line_cnt == 5:
...
def png_name_determination(ref_data, cv_data):
  ...
  for img_n, ocr_name in png_ocr_yield_name_from_image_variations(cv_data):
    ...
    if hit:
      if DEV == 2:
        print(f'det algo=score1, {img_n=} | {png_name_sorted_scores(scores)}')
      return hit, ocr_read
  if not (hit or cv_data[BAD_RESO]):
    hit = png_name_score_check(scores, readable_img_cnt, min_valid_perc, 1.01)
    if DEV == 2:
      print(f'det algo=score2, {img_n=} | {png_name_sorted_scores(scores, 9)}')
    if not hit:
      hit = png_name_fallback_algo(ocr_read)
      if DEV == 2:
        print(f'det algo=fb, {img_n=} | {png_name_sorted_scores(scores, 9)}')
  return hit, ocr_read
# -------------------------------------
...
def png_screenshot_values(date_str, img, ref_data):
  ...
  if DEV >= 1:
    print(f'{x_valid=}, {left=}{NL}{row_cnt=}, {rows=}{NL}{col_cnt=}, {cols=}')
    if DEV == 1:
      return {ROW_CNT: 0}
  if row_cnt == 0:
    return {ROW_CNT: 0}
...
def png_values_row_update(png_vals):
  ...
  for row_n in range(1, png_vals[ROW_CNT] + 1):
    if (ROW or ENDROW) and not (start_row < row_n <= e_row):
      continue
    print_progress_bar(png_vals[BAR], png_vals[ROW_CNT], row_n)
    png_vals[LOG_DATA][4] = png_vals[ROW_N] = row_n
    png_vals[AV_ARGS][0] = cv_vals[ROI]['orig'][0] = top = (
      png_vals[ROWS][row_n - 1] + 2
    )
    png_vals[AV_ARGS][1] = cv_vals[ROI]['orig'][1] = png_vals[ROWS][row_n] - 2
    cv_vals[ROI]['resize'][0] = cv_vals[RE_ROW][row_n - 1]
    cv_vals[ROI]['resize'][1] = cv_vals[RE_ROW][row_n] - cv_vals[RE_MA]
    cv_vals[NP] = png_vals[IMG][top + 2, cv_vals[RI]] in COLOR[NP]
    png_vals[NAME_ARGS][1] = cv_vals
    yield png_vals
  if DEV >= 1:
    print(''.center(80, '='))
# -------------------------------------
...
def reference_names_and_contract_data(df, kw_date):
  ...
  return (
    ref_names
    , reference_contract_list(kw_date, df_ref)
    , {name: sim.split(';') for name, sim in df_ref[SIM_NAM].items() if sim}
    , {*ref_names}
  )
# -------------------------------------
...
def tidy_screenshot_fn(original, fn_cf, idx_dict, log):
  ...
  png_filename = f'{current_day}_{idx_dict[current_day]}.png'
  if similarity != 100:
...
if __name__ == '__main__':
  ...
  *args, DEV, EIV, DAYS, FIDX, ROW, ENDROW, SAVE_REP = parser.values()
  if DEV >= 1:
    SAVE_REP = False
    print(''.center(80, '-'))
    print(f':::::   default cities:              {DEF_CITY}')
